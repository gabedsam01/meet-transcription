...
def _app(tmp_path, worker_repos):
    """Wire the app: auth bundle drives login; the worker bundle owns jobs.

    Admin bootstrap creates user id=1 in the auth bundle, matching the worker
    rows seeded by ``_seed_user1``.
    """
    return create_app(
        _web_settings(tmp_path),
        repositories=build_fake_repositories(),
        worker_repositories=worker_repos,
    )
...
def _seed_user1(repos):
    # Admin login creates user id=1; seed the worker-side ports for that id.
...
def test_jobs_page_handles_backend_unavailable_gracefully(tmp_path, monkeypatch):
    # No worker bundle injected; an unknown backend degrades gracefully (no 500).
