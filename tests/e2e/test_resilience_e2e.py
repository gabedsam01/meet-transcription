...
def test_failed_job_appears_in_ui_as_dead_letter(tmp_path):
    ...
    with TestClient(app) as client:
        login(client)
        client.post("/jobs/run-once", follow_redirects=False)
        # The worker fails transcription (Deepgram raises). job_max_attempts=1 makes
        # the unexpected error terminal in one pass (retry/backoff is unit-tested in
        # test_worker_processor.py), so the job dead-letters and shows as failed.
        run_worker_once(
            tmp_path, worker, drive=drive,
            deepgram=FakeDeepgramClient(fail=True), job_max_attempts=1,
        )
